( function( blocks, components, i18n, element){
	...
	registerBlockType('arforms/arforms-shortcode',{
		...
		insert:function(props){
			arfopenarfinsertform();
		},
		edit:function(props){
			window.arf_props_selected = '1';
			window.arf_props = props;
			var check_block_content = jQuery('#block-'+window.arf_props.clientId).find('.wp-block-arforms-arforms-shortcode').val();
			var check_block_content_length = jQuery('#block-'+window.arf_props.clientId).find('.wp-block-arforms-arforms-shortcode').length;
			if(props.name == 'arforms/arforms-shortcode')
			{
				if((check_block_content=='' || check_block_content==undefined || check_block_content=='undefined' || check_block_content_length==0)) {
					if( props.isSelected ){
						arfopenarfinsertform();
					} else {
						return 	el( 'textarea', {className:'wp-block-arforms-arforms-shortcode', style: blockStyle, onChange: function( content ) {
									props.setAttributes( { ARFShortcode: jQuery('#block-'+window.arf_props.clientId).find('.wp-block-arforms-arforms-shortcode').val() } );
								},}, props.attributes.ARFShortcode );
					}
				}
				return 	el( 'textarea', {className:'wp-block-arforms-arforms-shortcode', style: blockStyle, onChange: function( content ) {
							props.setAttributes( { ARFShortcode: jQuery('#block-'+window.arf_props.clientId).find('.wp-block-arforms-arforms-shortcode').val() } );
		...
		save:function(props){
			...
			if(window.arf_props!==undefined && window.arf_props!==null )
			{
				
				if(jQuery('#block-'+window.arf_props.clientId).find('.editor-block-list__block-html-textarea').is(':visible')==true)
				{
					props.attributes.ARFShortcode = jQuery('#block-'+window.arf_props.clientId).find('.editor-block-list__block-html-textarea').val();
				}
				
			}
			return props.attributes.ARFShortcode;
		}
	});
